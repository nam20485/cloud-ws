...
- [ ] (Rec) Run `ansible-lint` on the repo (catches style & common errors) — Benefit: early failure; Tradeoff: may require minor stylistic fixes.
- [ ] (Rec) `ansible-playbook --syntax-check ubuntu-cinnamon/workstation.yml` — Benefit: ensures YAML + task syntax; Tradeoff: none.
- [ ] (Rec) `ansible-playbook -i inventory.ini ubuntu-cinnamon/workstation.yml --list-tasks` — Benefit: scope preview; Tradeoff: none.

## 2. Dry Run & Safety Gates

- [ ] (Rec) Use `--check` mode (dry run) (some tasks may report changed incorrectly) — Benefit: safe preview; Tradeoff: not all modules fully support check mode.
...
- [ ] (MutEx)(Adv) Molecule + Delegated (local VM) for closer system fidelity — Benefit: real systemd & kernel modules; Tradeoff: more setup.

Additional:
- [ ] (Opt) Write Molecule verify step using Testinfra (check packages, services, rendered configs).
...
- [ ] (MutEx)(Opt) QEMU/KVM local VM with 4 extra virtio disks — Benefit: isolation; Tradeoff: more RAM/CPU.
...
- [ ] (Rec) Check service: `systemctl status dcvserver` — Benefit: immediate health.
- [ ] (Opt) `dcv list-sessions` & create a virtual session — Benefit: session layer test; Tradeoff: modest time.
...
- [ ] (Rec) Service status: `systemctl status parsecd` — Benefit: baseline.
- [ ] (Opt) Client connection & stats overlay to validate bitrate and latency — Benefit: functional QoE; Tradeoff: manual.
- [ ] (Adv) Automated screenshot diff (headless X) to detect rendering differences — Benefit: regression detection; Tradeoff: scripting overhead.

## 9. Root Migration Testing

Pick ONE initial approach (MutEx):
- [ ] (MutEx)(Rec) VM snapshot test: enable `migrate_root: true` → verify new root device after reboot — Benefit: safe & reversible; Tradeoff: requires snapshot tooling.
...
- [ ] (Rec) Confirm new `/` is LV via `lsblk` & `mount` — Benefit: correctness.
- [ ] (Rec) Re-run playbook (idempotent path) — Benefit: ensures no re-migration.

## 10. Monitoring & Metrics

- [ ] (Rec) Curl node exporter metrics: `curl localhost:9100/metrics | grep gpu_` — Benefit: quick validation.
- [ ] (Opt) Spin up ephemeral Prometheus via docker-compose to scrape metrics — Benefit: trending visibility; Tradeoff: container setup.
- [ ] (Adv) Add Grafana dashboard JSON (local container) — Benefit: visualization; Tradeoff: more components.
...
- [ ] (Rec) GitHub Actions: lint + syntax job (matrix of ansible versions) — Benefit: consistent gate.
- [ ] (Rec) GitHub Actions: Molecule (container) idempotency check — Benefit: automated regression detection.
- [ ] (Opt) Artifact upload of logs (`/proc/mdstat`, `lvs`, `nvidia-smi`) — Benefit: audit trail.
...
- [ ] (Opt) ansible-lint ruleset extension for security
- [ ] (Opt) Lynis baseline scan inside VM
- [ ] (Opt) Fail2ban & firewall rule validation
